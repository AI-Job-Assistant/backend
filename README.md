...
> 프론트엔드 ↔ 백엔드 연동 계약 문서  
> 최종 업데이트: 2026-07-24 · 작성: 백엔드 A (시현)
...
| 필드 | 값 | 설명 |
|---|---|---|
| `sessionType` | `"challenge"` / `"practice"` | **권장.** `challenge` = 도전 모드(질문 1개 + 통계 제외 자동 적용), `practice` = 정식 면접(질문 5개) |
| `mode` | `"스피킹"` / `"텍스트"` | 이 세션이 어떤 화면에서 진행됐는지 기록. 마이페이지 이력에 뱃지로 표시됨 |
| `interviewStyle` | `"압박"` | 압박 면접. 지원자의 판단·근거를 파고드는 도전적인 질문이 생성됨. 생략하면 일반 난이도 |
| `count` | `1` | 질문 1개만 생성. `sessionType: "challenge"`와 동일한 효과 (구버전 호환용) |

> `sessionType: "challenge"` · `mode: "도전"` · `count: 1` 중 **아무거나 하나만 보내도** 도전 모드로 처리된다.

**모드별 요청 예시**

```json
// 정식 텍스트 면접 (질문 5개)
{ "jobId": 102, "questionType": "경험행동형", "sessionType": "practice", "mode": "텍스트" }

// 정식 스피킹 면접 (질문 5개, 이력에 "스피킹" 뱃지)
{ "jobId": 102, "questionType": "직무기술형", "sessionType": "practice", "mode": "스피킹" }

// 도전 모드 (질문 1개) → mode는 자동으로 "도전"
{ "jobName": "AI 엔지니어", "questionType": "경험행동형", "sessionType": "challenge" }

// 압박 면접
{ "jobName": "AI 엔지니어", "questionType": "직무기술형", "sessionType": "practice", "interviewStyle": "압박" }
```

> ⚠️ **`practice`일 때 `mode`도 같이 보낼 것** — `sessionType: "practice"`만 보내면 백엔드가 텍스트/스피킹을 구분할 수 없어 이력 뱃지가 전부 "텍스트"로 표시된다.

> **도전 모드 주의** — 도전 모드 세션은 마이페이지의 **총 연습 횟수·평균 점수에서 제외**된다 (이력·잔디에는 표시됨).
...
- `extraTimeUsed`: **압박 면접 추가 시간 감점용** (선택). 추가 시간을 쓴 횟수(0/1/2)를 보내면 **1회당 3점, 최대 6점 감점**된다. 감점 시 개선점에 안내 문구가 자동으로 붙는다. 압박 면접에서만 보내면 됨 — 일반 면접은 생략(또는 0)하면 감점 없음.

**⚠️ 빈 답변도 그대로 보내면 된다 (400 안 남)**

`answer`가 빈 문자열이거나 공백만 있어도 **200으로 응답하며 0점 처리**된다. 무응답 = 0점이라는 실제 면접 기준을 따른 것. 프론트에서 빈 답변을 굳이 막을 필요 없고, 시간 초과로 넘어간 문항도 그대로 제출하면 된다.
...
> 또한 AI(Groq)가 일시적으로 실패해도 **500 대신 200 + 기본 피드백**을 반환한다. 이 API는 정상 요청이면 실패로 앱이 멈추지 않는다.
...
**GET `/api/departments`** — 회원가입 학과 드롭다운용. **성신여대 AI융합학부·컴퓨터공학과 4개만** 반환한다 (2026-07-24 교체 완료).

```json
[
  { "id": 1, "deptName": "AI융합학부(주전공)" },
  { "id": 2, "deptName": "AI융합학부(복수전공)" },
  { "id": 3, "deptName": "컴퓨터공학과(주전공)" },
  { "id": 4, "deptName": "컴퓨터공학과(복수전공)" }
]
```

- 응답 순서는 위 그대로(주전공 → 복수전공). 프론트에서 다시 정렬하지 말 것.
- 회원가입 시 선택한 `id`를 `departmentId`로 보내면 된다.

**GET `/api/jobs`** — `[{ id, jobCode, jobName, categoryName }]` 형식.
