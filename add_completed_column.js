...
const SQL = "ALTER TABLE interview_sessions ADD COLUMN completed BOOLEAN NOT NULL DEFAULT FALSE";

async function addTo(label, cfg) {
  const conn = await mysql.createConnection(cfg);
  try {
    await conn.query(SQL);
    console.log(`✅ ${label}: completed 컬럼 추가 완료`);
  } catch (err) {
    if (err.code === "ER_DUP_FIELDNAME") console.log(`ℹ️ ${label}: 이미 있음`);
